    ...
    , ui(new Ui::MainWindow)
{
    ...
    connect(dataBase_,
            ...
            this,
            &MainWindow::RcvSignalSendStatusConnectionToDB);
    connect(dataBase_,
            &DataBase::sig_SendListAirports,
            this,
            &MainWindow::RcvSignalSendListAirports);
    connect(dataBase_, &DataBase::sig_SendQueryFromDB, this, &MainWindow::RcvSignalSendQueryFromDB);
    connect(dataBase_,
            &DataBase::sig_SendDataToArrivals,
            this,
            &MainWindow::RcvSignalSendQueryFromDB);
    connect(dataBase_,
            &DataBase::sig_SendDataToDepartures,
            this,
            &MainWindow::RcvSignalSendQueryFromDB);
    connect(dataBase_, &DataBase::sig_SendDataPerYear, this, &MainWindow::RcvSignalSendDataPerYear);
    connect(dataBase_,
            &DataBase::sig_SendDataPerMonth,
            this,
            &MainWindow::RcvSignalSendDataPerMonth);
    connect(statistics_,
            &Statistics::sig_CloseStatistics,
            this,
            &MainWindow::RcvSignalCloseStatistics);
}
...
void MainWindow::RcvSignalSendListAirports(QSqlQueryModel *model)
{
    for (int i = 0; i < model->rowCount(); ++i) {
        ...
        airports_[model->data(model->index(i, 0)).toString()] = model->data(model->index(i, 1))
                                                                    .toString();
    }
}
...
void MainWindow::RcvSignalSendDataPerMonth(QSqlQueryModel *model)
{
    ...
    for (int i = 0; i < model->rowCount(); ++i) {
        data.insert(model->data(model->index(i, 1)).toInt(),
